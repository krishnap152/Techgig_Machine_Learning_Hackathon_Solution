# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 12:07:30 2018
...
WarrantyPeriod = 1
test_model = [[789,1099,28.20746133,25,1,67,12,45,23,2,232,3]]
y_predlrr = regressor.predict(test_model)
########## KNR
...
plt.title('RepairExpenditureDuringWarranty vs Estimated Service Cost (Training set)')
plt.xlabel('Repair Expenditure During Warranty')
plt.ylabel('Estimated Service Cost')
plt.show()

# Visualising the Test set results
plt.scatter(X_test['RepairExpenditureDuringWarranty'], y_test, color = 'red')
plt.plot(X_train['RepairExpenditureDuringWarranty'], regressor.predict(X_train), color = 'blue')
plt.title('RepairExpenditureDuringWarranty vs Estimated Service Cost (Test set)')
plt.xlabel('Repair Expenditure During Warranty')
plt.ylabel('Estimated Service Cost')
plt.show()
